...
import SpecificAppliesColumns from './SpecificAppliesColumns';

const SpecificApplies = () => {
  const { role,applies, isLoading, deleteApply } = useGlobalContext();
  ...
  if (applies.length < 1) {
    return (
      <EmptyContainer>
        <h5>
          Currently, there are no <span> APPLIES </span>
          to display
        </h5>
      </EmptyContainer>
    );
  ...
  return (
    <>
      <SpecificAppliesColumns />
      <Container>
        {applies.map((item) => {
          const { _id: id, company, position,name, createdAt } = item;
...
`;
export default SpecificApplies;
